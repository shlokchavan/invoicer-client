...
export class Slide {
    ...
    isSlidesEnd(counter: number) {
        ...
        let dataLength = this.dataForDOM ? this.dataForDOM.length : this.images ? this.images.length : this.cells.cellLength;

        if (this.carouselProperties.loop) {
            return false;
        } else {
            return (dataLength - counter + margin) < this.numberOfVisibleCells;
        }
    }
    ...
    isLightDOMMode(counter) {
        ...
        let remainderOfCells = length - this.overflowCellsLimit - this.numberOfVisibleCells;
