import { Component, ComponentFactoryResolver, ViewChild, AfterViewInit, ViewContainerRef, HostListener, EventEmitter } from '@angular/core';
...
export class RMModalComponent implements AfterViewInit {
  ...
  //Passed Data
  onActionClicked: EventEmitter<any>;
  Data:any;
  Center:boolean = false;
  Position:any = {Bottom:'10%', Right:'10%'}
  ModalBackdrop:boolean = false;
  ModalEscape:boolean = false;
  Animation:boolean = true;
  

  constructor(private componentFactoryResolver: ComponentFactoryResolver, private modals:DNDModalsComponent) {}
  ...
  ngAfterViewInit() {
    ...
    if(this.Center){
        ...
        //Set CSS
        (<HTMLElement>document.getElementById("modal-" + this.uniqueID)).style.bottom = this.Position.Bottom;
        (<HTMLElement>document.getElementById("modal-" + this.uniqueID)).style.right = this.Position.Right;
    }
  ...
  dragElement(elmnt) {
    ...
    var dragMouseDown = (e) => {
        ...
        if(this.Animation){
            document.getElementById(elmnt.id).style.opacity = "0.8";
        }
