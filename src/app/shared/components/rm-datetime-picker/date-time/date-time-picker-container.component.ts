...
@Component({
  ...
  selector: 'rm-date-time-container',
  ...
  implements OnInit, AfterContentInit, AfterViewInit
{
  ...
  /**
   * Handle click on set date time button
   */
  public onDateTimeSelected(event: any): void {
    this.dateChange.emit(this.pickerMoment);
    this.hidePicker$.next(null);
    this.picker.select(this.pickerMoment);
    event.preventDefault();
    return;
   ...
   */
  private dateSelectedInRangeMode(date: T): T[] | null | any {
   ...
   */
  private updateAndCheckCalendarDate(date: T): T | any {
    let result;
