...
@Component({
  ...
  viewProviders: [{ provide: DateAdapter, useClass: CustomDateAdapter }],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class QuerySearchItemComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    this.fields = this.querySearchService.fields;
    this.fields.subscribe((res) => {
      if (res.length > 0) {
        const onlyEqualsToOperators = res[0]["operators"].filter(
          (operator: any) => operator.symbol == "="
        );
        this.parameters = res[0]["parameterList"];
        this.visibleOperators = this.isUpdateClause
          ? onlyEqualsToOperators
          : res[0]["operators"];
      }
    });
  }
  ...
  get showValueField(): boolean {
    if (!!this.item) {
      return (
        !!this._item.condition &&
        !isNullOperator(this._item.condition.operatorKey) &&
        !!this._item.fieldName
      );
    }
  ...
  fieldChanged(isNameChange?) {
    if (isNameChange) {
      this.loadOperators();
      // Select Default Equal Operator if Update Clause Section
      if (this.isUpdateClause) {
        this.item.condition = this.visibleOperators.find(
          (operator: any) => operator.fieldType == this.item.type
        );
      } else {
  ...
  loadOperators() {
    ...
    if (this.visibleOperators)
      this.typedOperators = this.visibleOperators.filter((data) => {
        return (
          this.item.type == data.fieldType ||
          (this.item.type == "List" &&
            listOperatorsIds.includes(data.operatorId))
        );
    ...
    if (!!this.item && !!this.item.fieldName) {
      this.querySearchService.fields.subscribe((fields) => {
        ...
        if (!!field) {
          ...
          console.warn(
            `Could not find field with fieldName '${this.item.fieldName}'`,
            this
          );
        }
      });
  ...
  private updateFields(emit: boolean = true) {
    [...(this.singleFields || []), ...(this.stackedFields || [])].forEach(
      (field) => {
        if (
          field.item !== this.item ||
          field.item.fieldName !== this.item.fieldName ||
          field.item.type !== this.item.type
        ) {
          field.item = this.item;
        }
