...
import { AdvancedQuerySearchComponent } from "./advanced-query-search.component";
...
@NgModule({
  ...
  exports: [AdvancedQuerySearchComponent, QueryFieldDirective, AdvancedQueryFieldsDirective],
})
export class AdvancedQuerySearchModule {
  static forRoot(
    config?: QuerySearchConfig
  ): ModuleWithProviders<AdvancedQuerySearchModule> {
    const configuration = new QuerySearchConfiguration(config);

    return {
      ngModule: AdvancedQuerySearchModule,
      providers: [
        {
          provide: QUERY_SEARCH_CONFIG,
