...
import { AdvancedQueryFieldsDirective } from "./directives";
...
@Component({
  selector: "ngx-advanced-query-search",
  templateUrl: "advanced-query-search.component.html",
  styleUrls: ["advanced-query-search.component.scss"],
  animations: [...inOutAnimations],
})
export class AdvancedQuerySearchComponent
  implements AfterContentInit, ComponentCanDeactivate
{
  @Output() filterValid: EventEmitter<boolean> = new EventEmitter<boolean>();
  @Output() onFilterChanged: EventEmitter<void | any> = new EventEmitter<
    void | any
  >();
  @Output() filterLoaded: EventEmitter<SavedFilter> =
    new EventEmitter<SavedFilter>();
  ...
  @ContentChild(AdvancedQueryFieldsDirective)
  queryFields: AdvancedQueryFieldsDirective;
  ...
  ngAfterContentInit() {
    // console.log(this.queryFields.getFields());
  ...
  generate() {
    ...
    this.querySearchService.emitQuery(
      this.groups.map((g) => g.filterValue),
      this.queryBuilderId
    );
  }
  ...
  load(
    ...
    this.querySearchService.log("Loading pre-selected filter", {
      fieldName,
      operator,
      value,
    });
  ...
  public generateSavedFilter(
    ...
    if (Object.keys(queryRuleGroup).length === 0) {
      this.querySearchService.log(
        "Refusing to make filter from empty QueryRuleGroup",
        queryRuleGroup
      );
      return null;
    }
  ...
  filterChanged() {
    ...
    if (this.isUpdateClause) this.onFilterChanged.emit("updateClause");
    this.filterValid.emit(this.canGenerateFilter);
  }
  ...
  public get canGenerateFilter(): boolean {
    if (
      !!this.topGroup &&
      !!this.topGroup.group &&
      !!this.topGroup.group.filterValue &&
      Array.isArray(this.topGroup.group.filterValue.rules)
    ) {
      const isValidFilter = this.checkFilter(
        this.topGroup.group.filterValue.rules
      );
