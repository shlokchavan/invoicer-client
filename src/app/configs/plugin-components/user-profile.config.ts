import { IProfileDrawerConfig } from 'src/app/shared/components/rm-header/rm-header.model';
import { IUserDrawerConfig } from 'src/app/shared/components/rm-user-drawer/rm-user-drawer.model';

export class RMUserDrawerConfig implements IProfileDrawerConfig {
    isActive = false;
    isRightToleft = true;
    width = '450px';
}

export class _UserProfileConfig implements IUserDrawerConfig {
    image = '../../../assets/img/dummyProfile.jpg';
    // image = 'data:image/jpeg;base64,';
    firstName = 'clark';
    lastName = 'hogan';
    email = 'clark.hogan@arkedge.com';
    role = 'System Administrator';
    socialIcons = [
        { url: 'linkedinUrl', icon: 'linkedin' },
        { url: 'facebookUrl', icon: 'facebook' },
        { url: 'twitterUrl', icon: 'twitter' }
    ];
    actions = [
        {
            role: 'primary',
            title: 'My Account',
            action: 'profile',
        },
        {
            role: 'custom',
            customColor: 'red',
            title: 'Sign Out',
            action: 'logout',
        }
    ];
    profileActivity = {
        modifiedOn: '15th Sep 2019 05:23PM',
        lastLogin: '13th May 2019 03:45PM'
    };
    support = {
        email: {
            icon: 'email',
            title: 'Contact Support',
            action: 'mailto: support@invoicer.com',
        }
    };

}
